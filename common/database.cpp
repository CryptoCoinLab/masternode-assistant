...
bool Database::createMnTable()
{
    ...
    QString createTable = "CREATE TABLE t_master_node" \
                          "(mnhash TEXT NOT NULL PRIMARY KEY, "\
                          "seqdata BLOB NOT NULL)";
...
{
    QSqlQuery sql_query(sqldb);
    QString insert_sql = "insert into t_master_node " \
                         "(mnhash, seqdata) values (:mnhash,:seqdata)";
    ...
    if(!sql_query.exec())
    {
        qDebug() << "Error: Fail to insert table."
                 << sql_query.lastError();
        return false;
    }
    else
...
{
    QSqlQuery sql_query(sqldb);
    QString deleteMn = "DELETE FROM t_master_node WHERE mnhash=\""
            + qsKey + "\"";
    qDebug() << deleteMn;

    if(!sql_query.exec(deleteMn))
    {
        qDebug() << "Error: Fail to delete from table."
                 << sql_query.lastError();
        return false;
    }
    else
...
QByteArray Database::queryData(const QString &qsKey)
{
    QSqlQuery query(sqldb);
    QByteArray data = "";
    if (query.exec("SELECT * from t_master_node where mnhash='"
                   + qsKey + "'"))
    {
        while (query.next())
        {
...
{
    QSqlQuery query(sqldb);
    QString strSql = "update t_master_node set seqdata = " \
                     ":value where mnhash = :mnhash;";
