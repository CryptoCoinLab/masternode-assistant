...
typedef int (*plibssh2_session_supported_algs)(LIBSSH2_SESSION* session,int method_type,
            const char*** algs);
typedef LIBSSH2_API LIBSSH2_SESSION* (*plibssh2_session_init_ex)(LIBSSH2_ALLOC_FUNC((*my_alloc)),
            LIBSSH2_FREE_FUNC((*my_free)),LIBSSH2_REALLOC_FUNC((*my_realloc)), void *abstract);
typedef LIBSSH2_API int (*plibssh2_session_handshake)(LIBSSH2_SESSION *session,libssh2_socket_t sock);
typedef LIBSSH2_API const char* (*plibssh2_hostkey_hash)(LIBSSH2_SESSION *session,int hash_type);
typedef LIBSSH2_API int (*plibssh2_userauth_password_ex)(LIBSSH2_SESSION *session,const char *username,
            unsigned int username_len,const char *password,unsigned int password_len,
            LIBSSH2_PASSWD_CHANGEREQ_FUNC((*passwd_change_cb)));
typedef LIBSSH2_API LIBSSH2_CHANNEL* (*plibssh2_scp_recv2) (LIBSSH2_SESSION *session,const char *path,libssh2_struct_stat *sb);
typedef LIBSSH2_API int (*plibssh2_session_last_errno)(LIBSSH2_SESSION *session);
typedef LIBSSH2_API int (*plibssh2_session_last_error)(LIBSSH2_SESSION * session,char **errmsg,int *errmsg_len, int want_buf);
typedef LIBSSH2_API void (*plibssh2_session_set_timeout)(LIBSSH2_SESSION* session,long timeout);
typedef LIBSSH2_API ssize_t (*plibssh2_channel_read_ex)(LIBSSH2_CHANNEL *channel,int stream_id, char *buf,size_t buflen);
typedef LIBSSH2_API LIBSSH2_CHANNEL* (*plibssh2_scp_send_ex)(LIBSSH2_SESSION *session,const char *path, int mode,
            size_t size, long mtime,long atime);
typedef LIBSSH2_API ssize_t (*plibssh2_channel_write_ex)(LIBSSH2_CHANNEL *channel,int stream_id, const char *buf,size_t buflen);
...
typedef LIBSSH2_API int (*plibssh2_channel_get_exit_signal)(LIBSSH2_CHANNEL* channel,char **exitsignal,
            size_t *exitsignal_len,char **errmsg,size_t *errmsg_len,char **langtag,size_t *langtag_len);
typedef LIBSSH2_API int (*plibssh2_session_disconnect_ex)(LIBSSH2_SESSION *session,int reason,const char *description,const char *lang);
typedef LIBSSH2_API int (*plibssh2_session_free)(LIBSSH2_SESSION *session);
typedef LIBSSH2_API LIBSSH2_CHANNEL * (*plibssh2_channel_open_ex)(LIBSSH2_SESSION *session, const char *channel_type,
            unsigned int channel_type_len,unsigned int window_size, unsigned int packet_size,const char *message, unsigned int message_len);
typedef LIBSSH2_API int(*plibssh2_channel_process_startup)(LIBSSH2_CHANNEL *channel,const char *request,
            unsigned int request_len,const char *message,unsigned int message_len);
typedef LIBSSH2_API int (*plibssh2_session_block_directions)(LIBSSH2_SESSION *session);

class SshProto
{
public:
    static SshProto* initance();
public:
    ...
    plibssh2_session_set_timeout ssh2_session_set_timeout;
